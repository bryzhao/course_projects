 ...
 *
 * Bryan Zhao, November 22, 2015
 * Version 2 -- fixed bug: in line 77, changed (void) free (p); to (void) free (line[i]); since the value of p is discarded
 * once the for loop finishes executing, and memory space is still allocated to the current line, line[i], thus it needs to be
 * freed in memory to prevent the program from crashing or not having enough memory.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
